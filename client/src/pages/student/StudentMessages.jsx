  ...
  return (
    <AppLayout>
      <div className="page-header">
        <h1>Messages</h1>
        <p>Chat with your mentors in real-time.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '1rem', height: 'calc(100vh - 220px)' }}>
        ...
        <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '0.75rem', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            {error && <div style={{ padding: '1rem', color: '#ef4444', fontSize: '0.85rem' }}>{error}</div>}
            {!loading && conversations.length === 0 && (
              <div style={{ padding: '2rem', textAlign: 'center', color: '#64748b', fontSize: '0.85rem' }}>No conversations yet.</div>
            )}
            {conversations.map(c => (
              <div
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  background: active?.conversationId === c.conversationId ? '#eff6ff' : 'white',
                  borderBottom: '1px solid #f1f5f9',
                  transition: 'background 0.15s',
                }}
              >
                {c.other?.profilePhoto ? (
                  <img src={c.other.profilePhoto} alt="" style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                ) : (
          ...
          <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '0.75rem', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '0.875rem', borderTop: '1px solid #e2e8f0', display: 'flex', gap: '0.75rem' }}>
              <input
                ...
                style={{ flex: 1 }}
              />
              <button onClick={sendMessage} className="btn-primary" disabled={!text.trim()}>Send</button>
            </div>
          </div>
        )}
      </div>
